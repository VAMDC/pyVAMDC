...
def _make_request(endpoint: str, params: Dict[str, Any], base_url: str = API_BASE_URL) -> pd.DataFrame:
    """
    ...
    Parameters:
        ...
        base_url (str): Base URL of the RADEX API server
    ...
    url = f"{_base}{endpoint}?{query_string}" if query_string else f"{_base}{endpoint}"
...
def _cross_species(df_target: pd.DataFrame, df_collider: pd.DataFrame) -> pd.DataFrame:
    """
...
def _aggregate_results(results_list: List[Dict]) -> pd.DataFrame:
    """
...
def getRadex(
    ...
    Parameters:
        ...
        base_url (str): Base URL of the RADEX API server (default: API_BASE_URL from config)
    ...
    df_cross_species = _cross_species(target_df, collider_df)
    ...
    for row in rows:
        for db_collision in collision_nodes:
            for db_spectro in spectro_nodes:
                ...
                entries = _make_request("/entries/filter", params, base_url=base_url)
                if not entries.empty:
                    all_results.extend(entries.to_dict(orient="records"))

    df_raw = _aggregate_results(all_results)
    if df_raw.empty:
        return pd.DataFrame()
    ...
    return _downloadRadexFiles(df_raw[[c for c in keep if c in df_raw.columns]].copy(), output_dir=output_dir)
